...
PSOfunctional <- function(obj_function,
  ...
  while(iter < n_iter){
    ...
    dY <- 
      ...
      c2*runif(1)*matrix(w1,nrow = n_particles,ncol = N+1,byrow = TRUE)*
...
obj_diff <- function(obj,x,y){
  return(apply(abs(obj(x)-y),1,sum))
}
...
v <- PSOfunctional(sin_diff,a=0,b=6,c=-1.5,d=1.5,N=100,w=0.1,c1=0.9,c2=1,
                   n_particles = 200, n_iter = 2000,
                   delta.init=1/10,delta.v.init=1,delta1.r=1,delta2.r=1)
plot(v$X[1,],v$gbest)
print(v$gbest_obj)
```